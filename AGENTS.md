...
Each skill has one shared source under `plugins/<plugin-name>/skills/<skill-name>/`.
...
- Keep marketplace name `skills` and plugin names `core`, `workflow` and `thinking` unless the user requests a rename
- Keep names, descriptions, author, license and keywords aligned across manifests
- Keep README descriptions aligned with the matching plugin metadata
- Group README skills by plugin name, then sort each group by skill name
